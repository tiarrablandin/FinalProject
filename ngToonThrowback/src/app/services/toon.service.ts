    ...
    index() {
      return this.http.get<Toon[]>(this.url).pipe(
        catchError((err: any) => {
          ...
          return throwError(
            () => new Error (
              'ToonService.index(): error retrieving Toon List: ' + err
            )
          )
        }
        )
    ...
    create(toon: Toon) {
      return this.http.post<Toon>(this.url, toon).pipe(
        catchError((err: any) => {
          ...
          return throwError(
            () => new Error (
              'ToonService.create(): error creating Toon: ' + err
            )
          );
        })
      );
    ...
    update(toon: Toon) {
      return this.http.put<Toon>(this.url + '/' + toon.id, toon).pipe(
        catchError((err: any) => {
          ...
          return throwError(
            () => new Error (
              'ToonService.update(): error updating Toon: ' + err
            )
          );
        })
      );
    ...
    destroy(id: number) {
      return this.http.delete<Toon>(this.url + '/' + id).pipe(
        catchError((err: any) => {
          ...
          return throwError(
            () => new Error (
              'ToonService.destroy(): error deleting Toon: ' + err
            )
          );
        })
      );
